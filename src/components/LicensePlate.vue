<template>
    <div>
        ...
        <div class="py-2">
            <div class="flex gap-x-2 text-gray-300">
                ...
                <span class="text-sm whitespace-pre-line">{{ $t('messages.explanation') }}</span>
            </div>
        </div>
...
export default {
    ...
    data() {
        const gameActor = createActor(licensePlateMachine);
        return {
            keyboard: null,
            time: 30,
            maxTime: 30,
            ...
            // Actor instance
            gameActor: gameActor,
            // Start with the machine's initial state from snapshot
            current: gameActor.getSnapshot(),
            // Start with the machine's initial context from snapshot
            context: gameActor.getSnapshot().context,
        };
    },
    created() {
    ...
    watch: {
        'current.value': function (state) {
            if (state === 'finished' || (typeof state === 'object' && state.finished)) {
                this.saveScores();
    ...
    methods: {
        ...
        handleGuess(button) {
            this.gameActor.send({ type: 'GUESS', value: button });
        },
        newGuess() {
            this.gameActor.send({ type: 'CONTINUE' });
        },
        startGame() {
        ...
        restartGame() {
            ...
            this.gameActor.send({ type: 'RESTART' });
        },
        giveStarsScore() {
